 ...
 * vornonoi tesselation for a given set of vertex_indices.
 */
struct cell {
  /*! @brief Number of vertex_indices in cell */
  int count;

  /*! @brief Array of vertex_indices */
  double *vertices;

  /*! @brief Array of hilbert keys of vertex_indices */
...
/*! @brief Calculate the hilbert keys of the vertex_indices
 *
 * @param c Cell containing the vertex_indices
 */
static inline void cell_update_hilbert_keys(struct cell *c) {
  for (int i = 0; i < c->count; i++) {
    ...
    int nbits = 32;
    bits[0] =
        (c->vertex_indices[3 * i] - c->hs.anchor[0]) / c->hs.side[0] * (1ul << nbits);
    bits[1] = (c->vertex_indices[3 * i + 1] - c->hs.anchor[1]) / c->hs.side[1] *
              (1ul << nbits);
#else
 ...
 *
 * @param c Cell containing the vertex_indices to be sorted.
 */
...
/*! @brief Initialize a new cell with slightly randomized vertex_indices
 *
 * @param c Pointer to cell to be initialized
 * @param count Number of vertex_indices to add to cell
...
static inline void cell_init(struct cell *c, const int *count,
  ...
  /* slightly randomized vertex_indices */
...
/*! @brief Construct the delaunay triangulation of all the local vertex_indices (no
 * periodic boundaries).
 *
 * @param c Pointer to cell containing the vertex_indices to add to the delaunay
 * triangulation.
 */
static inline void cell_construct_local_delaunay(struct cell *c) {
  /* Add the local vertex_indices, one by one, in Hilbert order. */
  for (int i = 0; i < c->count; ++i) {
  ...
  /* we are done adding the original vertex_indices. We need to consolidate the
     indices of the original vertex_indices within the Delaunay tessellation, so
     that the tessellation knows that vertex_indices added from now on are ghosts.
   */
  delaunay_consolidate(&c->d);
}

/*! @brief Impose periodic boundaries by adding the necessary ghost vertex_indices
 *
 * Add ghosts to impose the periodic boundaries. These ghosts will be
 * periodic copies of the original vertex_indices that ensure that the incomplete
 ...
 * triangles that connect to original vertex_indices. Starting from an initial
 * search radius, we will iteratively add ghost vertex_indices and increase this
...
//  printf("count: %i\n", count);
  while (count > 0) {
    ...
    while (c->vertex_indices[3 * vi] < r) {
      if (i == c->count) break;
      ++i;
      if (c->vertex_indices[3 * vi] < old_r) {
        vi = c->r_sort_lists[0][i];
        continue;
      }
      delaunay_add_new_vertex(
          &c->d, c->vertex_indices[3 * vi] + c->hs.anchor[0] + c->hs.side[0],
          c->vertex_indices[3 * vi + 1]);
      vi = c->r_sort_lists[0][i];
    }
    /* add ghosts for the negative horizontal boundary */
    i = c->count - 1;
    vi = c->r_sort_lists[0][i];
    while (c->hs.anchor[0] + c->hs.side[0] - c->vertex_indices[3 * vi] < r) {
      if (i == -1) break;
      --i;
      if (c->hs.anchor[0] + c->hs.side[0] - c->vertex_indices[3 * vi] < old_r) {
        vi = c->r_sort_lists[0][i];
        continue;
      }
      delaunay_log("x: %g, old_r: %g, r: %g",
                   c->hs.anchor[0] + c->hs.side[0] - c->vertex_indices[3 * vi], old_r,
                   r);
      delaunay_add_new_vertex(
          &c->d, c->vertex_indices[3 * vi] - (c->hs.anchor[0] + c->hs.side[0]),
          c->vertex_indices[3 * vi + 1]);
    ...
    while (c->vertex_indices[3 * vi + 1] < r) {
      if (i == c->count) break;
      ++i;
      if (c->vertex_indices[3 * vi + 1] < old_r) {
        vi = c->r_sort_lists[1][i];
        continue;
      }
      delaunay_add_new_vertex(
          &c->d, c->vertex_indices[3 * vi],
          c->vertex_indices[3 * vi + 1] + c->hs.anchor[1] + c->hs.side[1]);
      vi = c->r_sort_lists[1][i];
    }
    /* add ghosts for the negative vertical boundary */
    i = c->count - 1;
    vi = c->r_sort_lists[1][i];
    while (c->hs.anchor[1] + c->hs.side[1] - c->vertex_indices[3 * vi + 1] < r) {
      if (i == -1) break;
      --i;
      if (c->hs.anchor[1] + c->hs.side[1] - c->vertex_indices[3 * vi + 1] < old_r) {
        vi = c->r_sort_lists[1][i];
        continue;
      }
      delaunay_add_new_vertex(
    ...
    while (c->vertex_indices[3 * vi] + c->vertex_indices[3 * vi + 1] < r * sqrt2) {
      if (i == c->count) break;
      ++i;
      if (c->vertex_indices[3 * vi] + c->vertex_indices[3 * vi + 1] < old_r * sqrt2) {
        vi = c->r_sort_lists[2][i];
        continue;
      }
      delaunay_add_new_vertex(
    ...
    i = c->count - 1;
    vi = c->r_sort_lists[2][i];
    while (c->hs.anchor[0] + c->hs.side[0] - c->vertex_indices[3 * vi] +
               c->hs.anchor[1] + c->hs.side[1] - c->vertex_indices[3 * vi + 1] <
           r * sqrt2) {
      if (i == -1) break;
      --i;
      if (c->hs.anchor[0] + c->hs.side[0] - c->vertex_indices[3 * vi] +
              c->hs.anchor[1] + c->hs.side[1] - c->vertex_indices[3 * vi + 1] <
          old_r * sqrt2) {
        vi = c->r_sort_lists[2][i];
        continue;
    ...
    while (c->vertex_indices[3 * vi] - (c->hs.anchor[1] + c->hs.side[1]) +
               c->vertex_indices[3 * vi + 1] <
           r * sqrt2) {
      if (i == c->count) break;
      ++i;
      if (c->vertex_indices[3 * vi] - (c->hs.anchor[1] + c->hs.side[1]) +
              c->vertex_indices[3 * vi + 1] <
          old_r * sqrt2) {
        vi = c->r_sort_lists[3][i];
        continue;
...
/*! @brief Relax this cells vertex_indices by moving them to the centroids of their
 * corresponding voronoi faces (Lloyds relaxation).
 *
 * After moving the vertex_indices, the hilbert keys and sort lists are updated and
...
static inline void cell_lloyd_relax_vertices(struct cell *c) {
  ...
  for (int i = 0; i < c->count; ++i) {
    int j = c->r_sort_lists[4][i];
#if defined(DIMENSIONALITY_2D)
    c->vertex_indices[3 * j] = c->v.cells[i].centroid[0];
    c->vertex_indices[3 * j + 1] = c->v.cells[i].centroid[1];
#else
